...
def gui():
    ...
    layout = [
        ...
        [sg.Button('Forward', size=(10, 1)), sg.Button('Down', size=(10, 1)), sg.Button('Calibrate', size=(10, 1))],
        [sg.Button('Snap', size=(10, 1)), sg.Button('Record', size=(10, 1), button_color='white on green'), ],
        ...
        [sg.Checkbox('Airframe coords', False, size=(40, 1), key='-AIRFRAME-')],
        ...
        [sg.Text('Gimbal Speed'),
         sg.Slider((0, 100), 40, 1, orientation='h', size=(40, 15), key='-GIMBAL SPEED-')],
        [sg.Text()],
...
def main(sock=None):
    ...
    while True:
        ...
        #     print(event, values)
            ...
            yaw_val = values['-YAW-']
            gimbal_cntrl.set_coordinate_sys(values['-AIRFRAME-'])
            print('Yaw', yaw_val)
            data = gimbal_cntrl.yaw(yaw_val)
            print(data[15:])
            sock.sendall(data)

        elif event in (['Calibrate']):
            print('quick_calibration, note this only works after powerup. Press and wait for 10 seconds')
            data = gimbal_cntrl.quick_calibration()
            print(data[15:])
            sock.sendall(data)
        ...
        elif event in (['Forward']):
            key = ord('f')

        elif event in (['Down']):
            key = ord('v')
        # if event in ('-UP-', '-LEFT-', '-RIGHT-', '-DOWN-'):
        elif 'GIMBAL' in event:
        ...
        gimbal_speed = values['-GIMBAL SPEED-']
